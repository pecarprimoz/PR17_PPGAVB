...
wb_new = Workbook()
ws1= wb_new.active
dest="../../nasi_podatki/primoz/nb_model_current.xlsx"
...
wb_my_own = load_workbook("../../nasi_podatki/primoz/nb_model_current.xlsx")
sr= wb_my_own["Matrika solaXpredmet"]
wb_new_two = Workbook()
ws_new= wb_new_two.active
dest="../../nasi_podatki/primoz/nb_model_current_only_numbers.xlsx"
...
for i in range(4,120): #ZGORN STOLPEC PREDMETU
    for j in range(2,528): #VSE SOLE, GLEJ C
        ...
        if predmet_v_stolpcu in predmeti_sole:
            ws_new[get_column_letter(i)+str(j)]=1
        else:
